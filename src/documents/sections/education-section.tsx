...
export function EducationPlaceItem({ educationPlace, theme }: EducationPlaceItemProps) {
  ...
  return (
    <EventItem
      ...
      blankLine={educationPlace.blankLine}
      theme={theme}
    >
      {educationPlace.courses &&
        Array.isArray(educationPlace.courses) &&
        educationPlace.courses.map(
          (course) => course && <EventHighlightItem key={course}>{course}</EventHighlightItem>
        )}
    </EventItem>
  );
}
    ...
    <EventsSection theme={theme} title={t("education")}>
      {education.map(
        (educationPlace, index) =>
          educationPlace && (
            <EducationPlaceItem key={index} theme={theme} educationPlace={educationPlace} />
          )
      )}
    </EventsSection>
  );
