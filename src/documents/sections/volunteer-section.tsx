...
export function VolunteeringItem({ volunteering, theme }: VolunteeringItemProps) {
  ...
  return (
    <EventItem
      ...
      blankLine={volunteering.blankLine}
      theme={theme}
    >
      {volunteering.highlights &&
        ...
        volunteering.highlights.map(
          (highlight) =>
            highlight && <EventHighlightItem key={highlight}>{highlight}</EventHighlightItem>
        )}
    </EventItem>
  );
}
...
export function VolunteerSection({ volunteer, theme }: SectionProps) {
  ...
  return (
    <EventsSection theme={theme} title={t("otherinfo")}>
      {volunteer.map(
        (volunteering, index) =>
          volunteering && <VolunteeringItem key={index} theme={theme} volunteering={volunteering} />
      )}
    </EventsSection>
  );
}
